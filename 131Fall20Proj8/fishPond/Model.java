...
public class Model {
			...
			if (curr.getSize() > Plant.MAX_PLANT_SIZE) {
				...
				if (count > 0) {
					...
					for (int j = 0; j < 8; j++)
						if (freeSpace[j])
							plants.add(
									new Plant(r + dy[j], c + dx[j], newSize));
	...
	public boolean isSpaceAvailable(int r, int c) {
		for (Fish fishes : fish) {
			if (r == fishes.getRow() && c == fishes.getCol()) {
				return false;
			}
		}
		for (Plant plant : plants) {
			if (r == plant.getRow() && c == plant.getCol()) {
				return false;
			}
		}
	...
	public Model(Model other) {
		...
		for (int index = 0; index < landscape.length; index++) {
			for (int col = 0; col < landscape[index].length; col++) {
				landscape[index][col] = other.landscape[index][col];
			}
		}

		fish = new ArrayList<Fish>();
		for (Fish fishes : other.fish) {
			fish.add(new Fish(fishes));
		}
		plants = new ArrayList<Plant>();
		for (Plant plant : other.plants) {
	...
	public void removeDeadFish() {
		for (int index = 0; index < fish.size(); index++) {
			if (fish.get(index).isAlive() == false) {
				fish.remove(fish.get(index));
				index--;
			}
		}
	}
	...
	public void removeDeadPlants() {
		for (int index = 0; index < plants.size(); index++) {
			if (plants.get(index).isAlive() == false) {
				plants.remove(plants.get(index));
				index--;
			}
		}
	}
	...
	private boolean fishIsSurroundedByRocks(Fish f) {
		int row = f.getRow();
		int cols = f.getCol();
		if (landscape[row - 1][cols] == ROCK && landscape[row + 1][cols] == ROCK
				&& landscape[row][cols - 1] == ROCK
				&& landscape[row][cols + 1] == ROCK) {
			return true;
		} else {
			return false;
	...
	public void turnFish() {
		for (Fish fishes : fish) {
			if (fishes.isAlive() == true) {
				if (fishIsSurroundedByRocks(fishes) == true) {
					continue;
				} else if (fishes.getDirection() != Fish.DOWN
						&& fishes.getDirection() != Fish.UP
						&& fishes.getDirection() != Fish.LEFT
						&& fishes.getDirection() != Fish.RIGHT) {
					throw new IllegalFishDirectionException(
							fishes.getDirection());

				}
				while ((fishes.getDirection() == Fish.DOWN
						&& landscape[fishes.getRow() + 1][fishes
								.getCol()] == ROCK)
						|| (fishes.getDirection() == Fish.UP
								&& landscape[fishes.getRow() - 1][fishes
										.getCol()] == ROCK)
						|| (fishes.getDirection() == Fish.LEFT
								&& landscape[fishes.getRow()][fishes.getCol()
										- 1] == ROCK)
						|| (fishes.getDirection() == Fish.RIGHT
								&& landscape[fishes.getRow()][fishes.getCol()
										+ 1] == ROCK)) {
					fishes.setRandomDirection();
		...
		for (Fish fishes : fish) {
			if (fishes.isAlive() == true) {
				if (fishIsSurroundedByRocks(fishes) == true) {
					...
					fishes.move();
					for (Plant plant : plants) {
						if (plant.getCol() == fishes.getCol()
								&& plant.getRow() == fishes.getRow()
								&& plant.isAlive() == true) {
							fishEatPlant(fishes, plant);

						}
					}
					/*
					 * for (int index = 0; index < fish.size(); index++) { if
					 * (fish.indexOf(fishes) != index &&
					 * fish.get(index).getRow() == fishes.getRow() &&
					 * fish.get(index).getCol() == fishes.getCol()) {
					 * fishes.fight(fish.get(index)); } }
					 */
					for (Fish fishe : fish) {
						if (fishes.getRow() == fishe.getRow()
								&& fishes.getCol() == fishe.getCol()
								&& fishes != fishe) {
							fishes.fight(fishe);
	...
	public void addPlant(Plant p) {
		int count = 0;
		for (Plant plant : plants) {
			if (plant.getRow() == p.getRow() && plant.getCol() == p.getCol()) {
				count++;
			}
		}
	...
	public void addFish(Fish f) {
		int count = 0;
		for (Fish fishes : fish) {
			if (fishes.getRow() == f.getRow()
					&& fishes.getCol() == f.getCol()) {
				count++;
			}
		}
	...
	public ArrayList<Fish> getFish() {
		ArrayList<Fish> copyFish = new ArrayList<Fish>(fish);
		return copyFish;
	...
	public ArrayList<Plant> getPlants() {
		ArrayList<Plant> copyPlant = new ArrayList<Plant>(plants);
		return copyPlant;
	}
